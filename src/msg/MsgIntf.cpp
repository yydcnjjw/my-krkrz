...
#define TVP_MSG_DECL_NULL(name)                                                \
    tTJSMessageHolder name(TJS_W(#name), NULL, false);
...
ttstr TVPGetAboutString(void) {
    TVPGetVersion();
    tjs_char verstr[100];
    TJS_snprintf(verstr, sizeof(verstr) / sizeof(tjs_char),
                 TJS_W("%d.%d.%d.%d"), TVPVersionMajor, TVPVersionMinor,
                 TVPVersionRelease, TVPVersionBuild);

    tjs_char tjsverstr[100];
    TJS_snprintf(tjsverstr, sizeof(tjsverstr) / sizeof(tjs_char),
                 TJS_W("%d.%d.%d"), TJSVersionMajor, TJSVersionMinor,
                 TJSVersionRelease);

    return TVPFormatMessage(TVPReadAboutStringFromResource().c_str(), verstr,
                            tjsverstr) /*+ TVPGetImportantLog()*/;
}
//---------------------------------------------------------------------------
...
ttstr TVPGetVersionInformation(void) {
    ...
    //     version.AsStdString(), tjs_string(TJS_W("%DATE%")),
    //     tjs_string(TVPCompileDate));
    // str = ApplicationSpecialPath::ReplaceStringAll(
    //     str, tjs_string(TJS_W("%TIME%")), tjs_string(TVPCompileTime));
    // return ttstr(str);
    return verstr;
}
//---------------------------------------------------------------------------
