...
void TVPAddLog(const ttstr &line) {
    GLOG_D(codecvt::utf_to_utf<char>(line.AsStdString()).c_str());
}

void TVPExecuteExpression(const ttstr &content, iTJSDispatch2 *context,
                          tTJSVariant *result) {
    krkrz::TJS2NativeScripts::get()->exec(content.AsStdString(), context,
                                          result);
}
...
void tTVPScenarioCacheItem::LoadScenario(const ttstr &name, bool isstring) {
    ...
    if (isstring) {
        ...
        try {
            Buffer = codecvt::utf_to_utf<char16_t>(krkrz::TJS2NativeStorages::get()
            ...
            if (CurLineStr[CurPos] == TJS_W('*')) {
                ...
                if (!RecordingMacro) {
                    ...
                    if (dsp) {
                        ...
                        tTJSVariant *psrc = &src;
                        DicAssign->FuncCall(0, NULL, NULL, NULL, 1, &psrc,
                                            DicObj);
                    }
                    tTJSVariant tag_val(tagname);
                    DicObj->PropSetByVS(TJS_MEMBERENSURE,
                ...
                if (entity) {
                    ...
                    if (args) {
                        ...
                        if (vp) {
                            ...
                            args->PropGet(0, name.c_str(), NULL, &ValueVariant,
                                          args);
                            if (ValueVariant.Type() == tvtVoid)
                                ValueVariant = ttstr(vp + 1);
                        } else {
                            args->PropGet(0, value.c_str(), NULL, &ValueVariant,
                                          args);
                        }
...
tTJSNC_KAGParser::tTJSNC_KAGParser() : inherited(TJS_W("KAGParser")) {
...
iTJSDispatch2 *TVPCreateNativeClass_KAGParser() { return new tTJSNC_KAGParser; }
